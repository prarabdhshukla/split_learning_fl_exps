...
class BaseTrainer:
  ...
  def eval(self,
    ...
    if self.args['is_regression']:
      ...
      for t in range(self.num_clients):
        ...
        if t==2 and round_idx==400:
          print(self.pred_fn(local_params[t],self.x_test[t]))
          print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
          print(self.y_test[t])
          print('****************************')
          print(losses[t][2])
  ...
  def eval_odd(self,
    ...
    for t in range(self.num_clients):
      sample_losses=[]
      ae=AutoEncoder(self.x_train[t].shape[0],self.x_train[t].shape[1])
      ae.train(self.x_train[t])
      train_loss = self.data_loss_fn(local_params[t], (self.x_train[t], self.y_train[t]))
      val_loss = self.data_loss_fn(local_params[t], (self.x_val[t], self.y_val[t]))
      for j in range(self.x_test[t].shape[0]):
        recons_loss=ae.recons_loss(np.array([self.x_test[t][j]]))
        if recons_loss<threshold:
          #in distribution
          test_loss=self.data_loss_fn(local_params[t],(self.x_test[t][j],self.y_test[t][j]))
          sample_losses.append(test_loss)
        else:
          #out of distribution, use global
          test_loss=self.data_loss_fn(global_params[t],(self.x_test[t][j],self.y_test[t][j]))
          sample_losses.append(test_loss)
      losses.append((train_loss,val_loss,sum(sample_losses)/len(sample_losses)))

    losses=np.array(losses).astype(float)
    avg_loss,std_loss=np.mean(losses,axis=0), np.std(losses,axis=0)
